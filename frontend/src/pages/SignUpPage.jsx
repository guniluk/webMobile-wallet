...
export default function SignUpPage() {
  ...
  const handleSignUp = async (e) => {
    ...
    try {
      console.log('👉 [Web] signUp.create 시도 중... 이메일:', email);
      await signUp.create({ emailAddress: email, password });
      
      console.log('👉 [Web] signUp.create 성공! OTP 코드 전송 중...');
      await signUp.verifications.sendEmailCode();
      setPendingVerification(true);
    } catch (err) {
      console.error('❌ [Web] 회원가입 에러:', err);
      setError(err.errors?.[0]?.message || '회원가입 요청에 실패했습니다.');
    } finally {
      setLoading(false);
    }
  };

  // 2단계: 인증코드 검증 및 최종 가입 완료
  const handleVerify = async (e) => {
    ...
    try {
      console.log('👉 [Web] signUp.verifications.verifyEmailCode 검증 시도 중...');
      await signUp.verifications.verifyEmailCode({ code: verificationCode });

      if (signUp.status === 'complete') {
        console.log('✅ [Web] 가입 인증 완료! 세션 활성화 중...');
        await setActive({ session: signUp.createdSessionId });
        navigate('/');
      } else {
        setError('인증을 완료했으나 가입 상태가 완료되지 않았습니다.');
      }
    } catch (err) {
      console.error('❌ [Web] 가입 인증 에러:', err);
      setError(err.errors?.[0]?.message || '인증 코드가 일치하지 않거나 만료되었습니다.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-gray-500">
              {pendingVerification ? '보안인증 코드를 입력해주세요' : '가계부 지갑 서비스 회원가입'}
            </p>
          </div>
            ...
            <form onSubmit={handleVerify} className="space-y-5">
              <div className="form-control">
                <div className="flex flex-col items-center justify-center bg-blue-50 rounded-2xl p-4 mb-4 border border-blue-100 text-center">
                  ...
                  <span className="text-sm font-bold text-blue-800">이메일 인증코드 발송됨</span>
                  <span className="text-xs text-blue-600 mt-1">입력하신 이메일({email})로 6자리 코드가 전송되었습니다.</span>
                </div>
